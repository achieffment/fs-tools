# examples — песочница синхронизации
...
./sync.sh examples --dry-run          # Linux/macOS (терминал)
./sync.command examples --dry-run     # macOS (двойной клик в Finder)
./sync.bat examples --dry-run         # Windows (через WSL/cwrsync)
```

Эквивалент напрямую: `python sync_fs.py examples --dry-run`.
...
git clean -fd examples        # уберёт _fs-archive/, .fs-log и прочие следы прогона
```
