...
- **`[[backup]]` «vault»** — offload с `after_push = "archive"` и `verify = true`.
  При offload сохраняются только якорные include-каталоги; промежуточные уровни
  остаются только при отдельном якорном include-паттерне. Удаляется/переносится
  только подтверждённо переданное содержимое. Для одного якоря сохраняется ближайший
  уровень (вложенный одноимённый каталог не сохраняется автоматически).
...
Статус: ok. Синхронизация каталога /path/to/examples/syncher завершена успешно.
Сводка: профилей: 2; передано: 6; удалено: 2; выгружено: 0; ошибок: 0; блокировок: 0.
```
