...
def test_archive_pruned_on_intermediate_star(
    ...
    root = make_tree(
        [
            "Activities/Web/Projects/Work/Fabrikam/widgets.example/Data/",       # обычный проект, нет project.md
            "Activities/Web/Projects/Work/Fabrikam/_Archive/acoustic.example/Data/",  # _Archive на позиции проекта
        ]
    )
    rule_text = (
    ...
    assert "Activities/Web/Projects/Work/Fabrikam/_Archive/acoustic.example/Data/project.md" not in missing
    # У реального проекта отсутствие project.md репортится.
    assert missing == ["Activities/Web/Projects/Work/Fabrikam/widgets.example/Data/project.md"]
...
def test_canonical_oracle_exactly_four(make_tree: Callable[[Iterable[str]], Path]) -> None:
    ...
    root = make_tree(
        [
            ...
            f"{P}Addl/crm.example.com/Back/", f"{P}Addl/crm.example.com/Data/",
            f"{P}Addl/safegrid.example/Back/", f"{P}Addl/safegrid.example/Data/",
            f"{P}Addl/shop.example.com/Back/", f"{P}Addl/shop.example.com/Data/",
            f"{P}Addl/_Archive/aero.example/Back/",   # нет Data -> нарушение
            f"{P}Addl/_Archive/analytics.example.net/Back/", f"{P}Addl/_Archive/analytics.example.net/Data/",
            f"{P}Addl/_Archive/andromeda.example/Back/", f"{P}Addl/_Archive/andromeda.example/Data/",
            f"{P}Self/",                       # категория без проектов -> нет якорей
            f"{P}Work/Contoso/",                # организация без проектов -> нет якорей
            f"{P}Work/Fabrikam/_Archive/acoustic.example/",        # нет ни Back, ни Data -> 2 нарушения
            f"{P}Work/Fabrikam/_Archive/partners.example/Back/", f"{P}Work/Fabrikam/_Archive/partners.example/Data/",
            f"{P}Work/Fabrikam/_Archive/studio.example/Back/", f"{P}Work/Fabrikam/_Archive/studio.example/Data/",
        ]
    )
