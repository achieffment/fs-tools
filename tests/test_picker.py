"""Тесты выбора каталога и предзаполнения папки по умолчанию (normalizer.picker)."""
import os
import subprocess

import pytest

from normalizer import picker
...
# --------------------------------------------------------------------------- #
# pick_directory (ветка обычного Linux)
# --------------------------------------------------------------------------- #
@pytest.fixture()
...
def test_pick_directory_windows_uses_cwd_as_initial(monkeypatch, tmp_path, _force_windows):
    ...
    monkeypatch.setattr(picker, "_win_folder_dialog", fake_dialog)
...
# --------------------------------------------------------------------------- #
# pick_directory (ветка WSL: round-trip конвертации путей)
# --------------------------------------------------------------------------- #
@pytest.fixture()
...
def test_pick_directory_wsl_converts_cwd_and_roundtrips(monkeypatch, _force_wsl):
    ...
    monkeypatch.setattr(picker, "_win_folder_dialog", fake_dialog)
    monkeypatch.setattr(picker, "_to_wsl_path", lambda p: "/home/user/chosen")
...
def test_pick_directory_wsl_selects_windows_folder(monkeypatch, _force_wsl):
    ...
    monkeypatch.setattr(picker, "_win_folder_dialog", lambda initial=None: r"C:\Users\me")
    seen: dict[str, str] = {}
    ...
    monkeypatch.setattr(picker, "_to_wsl_path", fake_to_wsl)
    result = picker.pick_directory()
...
def test_pick_directory_windows_selects_wsl_folder(monkeypatch, _force_windows):
    ...
    monkeypatch.setattr(picker, "_win_folder_dialog", lambda initial=None: unc)
    monkeypatch.setattr(
        picker, "_to_wsl_path", lambda p: pytest.fail("конвертация не нужна на Windows")
    )
    assert picker.pick_directory() == unc


# --------------------------------------------------------------------------- #
# pick_directory (ветка macOS)
# --------------------------------------------------------------------------- #
@pytest.fixture()
...
def test_pick_directory_mac_uses_cwd_as_initial(monkeypatch, tmp_path, _force_mac):
    ...
    monkeypatch.setattr(picker, "_mac_folder_dialog", fake_dialog)
    assert picker.pick_directory() == "/Users/me/sel"
    assert captured["initial"] == os.getcwd()
...
def test_pick_directory_mac_falls_back_to_prompt(monkeypatch, tmp_path, _force_mac):
    ...
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(picker, "_mac_folder_dialog", lambda initial=None: None)
    monkeypatch.setattr("builtins.input", lambda: "")
    assert picker.pick_directory() == os.getcwd()
...
def test_win_folder_dialog_none_when_script_missing(monkeypatch, tmp_path):
    monkeypatch.setattr(picker.shutil, "which", lambda name: "/fake/powershell.exe")
    monkeypatch.setattr(picker, "_PICK_FOLDER_PS1", tmp_path / "missing.ps1")
