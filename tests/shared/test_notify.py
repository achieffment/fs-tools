...
def test_posts_text_and_bearer(monkeypatch: pytest.MonkeyPatch, isolated: None) -> None:
    ...
    rec = _Recorder()
    monkeypatch.setattr("requests.post", rec)
    assert notify.send_webhook("есть ошибки", url_key=_URL_KEY, tok_key=_TOK_KEY, logger=_LOG) is True
...
def test_load_webhook_config(monkeypatch: pytest.MonkeyPatch, isolated: None) -> None:
    monkeypatch.setenv(_URL_KEY, "https://example.com/hook")
    monkeypatch.setenv(_TOK_KEY, "secret")
    assert notify.load_webhook_config(_URL_KEY, _TOK_KEY) == ("https://example.com/hook", "secret")
