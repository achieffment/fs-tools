@echo off
chcp 65001 >nul
REM Обёртка для Windows: при первом запуске готовит .venv и зависимости, затем запускает sync_fs.py.
setlocal
set "fold=%~dp0"
set "pyex=%fold%.venv\Scripts\python.exe"

if not exist "%pyex%" goto setup
"%pyex%" -c "import requests, dotenv" 1>nul 2>nul
