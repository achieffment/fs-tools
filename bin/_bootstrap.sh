...
# editable-установку с обоими extra. Подключается обёртками через `source`; сам по
# себе ничего не запускает. После вызова `_fs_tools_bootstrap "<корень>"` задаёт:
#   FS_TOOLS_HOME — корень проекта (нужен для поиска единого .env);
#   FS_TOOLS_VBIN — каталог bin виртуального окружения (где лежат fs-normalizer/fs-checker).
...
_fs_tools_bootstrap() {
    ...
    if [ ! -x "$pyex" ] || ! "$pyex" -c "import fs_tools, pathspec, unidecode, requests, dotenv" >/dev/null 2>&1; then
        ...
        if ! "$pyex" -m pip install -e "$root[normalizer,checker]"; then
