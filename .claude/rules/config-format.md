...
- Передача: `checksum`, `compress`, `partial_progress` (bool), `bwlimit` (строка),
  `ssh_opts` (список строк), `preserve_perms` (bool, дефолт `true`; `false` → rsync
  `--no-perms`), `chmod` (строка, опционально; синтаксис rsync `--chmod`, напр.
  `D755,F644` — явные права на приёмнике). С `--no-perms` rsync применяет `--chmod`
  только к переданным файлам; для drvfs достаточно одного `chmod`.
