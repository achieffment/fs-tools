...
# Матрица изменений и распределение по пакетам

> Claude-эквивалент [`.cursor/rules/docs-consistency-matrix.mdc`](../../.cursor/rules/docs-consistency-matrix.mdc). Применяется при работе с `src/fs_tools/**`/`tests/**`/`examples/**`.

Детализация раздела «Роли артефактов» [`docs-consistency.md`](docs-consistency.md)
для конкретных типов изменений в коде/тестах/examples.
...
| Новое/удалённое правило Cursor/Claude                                             | `tests/shared/test_rules_consistency.py`                                                                                                                            | [`rules-sync.md`](rules-sync.md) — карта соответствия (обе версии); `AGENTS.md`/`CLAUDE.md`                                                            |
