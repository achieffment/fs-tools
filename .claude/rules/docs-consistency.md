...
2. Полный цикл: `pytest`, `pylint --recursive=y src tests/*`, `ruff check .`,
   `mypy --strict -p fs_tools`;
3. Правила по «Матрице изменений» — в обоих каталогах (`.claude/rules/*.md` и
   `.cursor/rules/*.mdc`);
...
- [ ] `pytest`, `pylint --recursive=y src tests/*`, `ruff check .`,
