...
# Входные данные через env: FSNORM_INITIAL (стартовая папка), FSNORM_TITLE (заголовок).
...
$owner.Activate()
try {
  $path = [FsNorm.Picker]::Pick($env:FSNORM_INITIAL, $env:FSNORM_TITLE, $owner.Handle)
} finally {
  $owner.Close()
  $owner.Dispose()
}
