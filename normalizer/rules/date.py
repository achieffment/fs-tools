...
class DateRule(Rule):
    ...
    #
    # ВАЖНО: '_' — собственный токен-разделитель нормализатора (он сам ставит его
    # между токенами: 'dump_2020-...', 'name_2020'). В двухполевой 'mmyy' '_' НЕ
    # считается разделителем: иначе ведущий ноль индекса рядом с годом ('5 ' ->
    # LeadingZero -> '05_2020-...') на повторном прогоне читался бы как MM_YYYY и
    # ломал уже валидную дату (05_2020-05-20 -> 2020-05-00_05-20). Это коллизия с
    # очень частым 'index_year'/'name_year'. В трёхполевой 'full' '_' оставлен:
    # 'YYYY_MM_DD' однозначна и не конфликтует, так что 2020_05_20 -> 2020-05-20.
    _RE = re.compile(
        r"""
        ...
        (?:
          ...
          | (?P<mmyy>                   # месяц+год: MM.YYYY или YYYY.MM
                \d{1,2}[./-]\d{4,4}     # '_' НЕ разделитель здесь (см. выше)
              | \d{4,4}[./-]\d{1,2}
            )
          | (?P<year>                   # только год
                \d{4,4}
    ...
    # Делит найденную дату на части. Должен покрывать все разделители групп выше,
    # включая '_' (его допускает трёхполевая 'full': 2020_05_20). Не убирай '_'
    # «для симметрии» с 'mmyy' — сломается разбор подчёркнутой полной даты.
    _SP = re.compile(r"[._/\-]")
