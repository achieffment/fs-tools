...
Конфигурация — из единого `.env` проекта (`FSSYN_WEBHOOK_URL` / `FSSYN_WEBHOOK_TOK`).
...
`fs-syncher` импортировалась и работала без extra `syncher`.
...
def send_webhook(text: str) -> bool:
    ...
    try:
        import requests
    except ImportError:                             # без extra syncher отправка недоступна
