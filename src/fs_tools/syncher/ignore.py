"""Трансляция правил include/exclude из .fs-sync.toml в фильтры rsync.
...
Служебные артефакты (`.fs-sync.toml`, `.fs-log`, `.env`) исключаются всегда: иначе
они уйдут на сервер, а меняющийся `.fs-log` сломает идемпотентность (повторный прогон
давал бы вечный diff).
"""
from __future__ import annotations

# Имена собственных артефактов утилиты — никогда не передаются на сервер.
ARTIFACTS: tuple[str, ...] = (".fs-sync.toml", ".fs-log", ".env")
...
def auto_exclude_filters() -> list[str]:
    ...
    return ["- /.fs-sync.toml", "- /.fs-log", "- .env"]
