...
- **проверка структуры** (`fs-checker`) — рекурсивно сверяет дерево с правилами `.fs-chk`
...
- **синхронизация** (`fs-syncher`) — односторонняя синхронизация каталога с сервером
  (ПК → сервер) через внешний `rsync` по декларативному `.fs-syn.toml`. Состав
...
- **проверка схемы** (`fs-schemer`) — рекурсивно сверяет базу знаний с декларативными
  группами `.fs-sch.toml`: обязательные/опциональные служебные файлы, точный текст
...
Положите файл `.fs-nrm` **в нормализуемый каталог** (как `.gitignore` в корне репозитория).
...
|-----|---------|
| 0 | нарушений нет |
| 1 | ошибка запуска: каталог не выбран / не найден / не каталог, нет `.fs-chk` |
...
Рекурсивно сверяет базу знаний с декларативными **группами** `.fs-sch.toml`: дерево
...
служебными файлами и «обычными» файлами группы. Конфиг `.fs-sch.toml` (без ведущей
...
`.fs-sch.toml` в корне проверки под `loose_file` не попадает — он скрытый (ведущая
точка), как и `.fs-syn.toml`/`.fs-chk`.
...
|-----|---------|
| 0 | нарушений нет |
| 1 | ошибка запуска: каталог не выбран / не найден / не каталог, нет/невалиден `.fs-sch.toml` |
...
`rsync`: читает и валидирует `.fs-syn.toml`, транслирует правила include/exclude в
...
fs-syncher /path/to/dir                   # все профили из .fs-syn.toml
...
`rsync` обрабатывает фильтры по принципу **«первое совпадение»**, поэтому порядок:
    ...
    1. безусловные артефакты:  - /.fs-syn.toml, - /.fs-log, - .env
    2. include (override):     + <pattern>
    3. exclude:                - <pattern>

Артефакты `.fs-syn.toml`, `.fs-log`, `.env` исключаются **всегда** и не возвращаются
...
| 1 | ошибка запуска: нет каталога/`.fs-syn.toml`, ошибка валидации, нет `rsync` (или `ssh` при SSH-цели) |
...
правилам + скрипты отката `reset.*`), `examples/checker/` (дерево с `.fs-chk` и
...
(`Warehouse/.fs-sch.toml` с намеренными нарушениями F1–F15). Подробности — в README
