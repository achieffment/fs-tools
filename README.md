...
Пути сопоставляются **относительно нормализуемого каталога** (как `.gitignore` в корне репозитория). Пустые строки и комментарии (строка, начинающаяся с `#`) игнорируются. Пример: исключаем архивы и сборки, но возвращаем папки `Data` и не-скрытые файлы `*.keep` внутри них.
