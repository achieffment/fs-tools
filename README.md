...
- **синхронизация** (`fs-syncher`) — односторонняя синхронизация каталога с сервером
  ...
  задаются профилями. Режим неинтерактивен (без диалога выбора каталога).
...
обычный Linux — ввод пути в терминале (режим синхронизации диалог не использует).
...
Режим **неинтерактивен**: путь к каталогу — позиционный аргумент; без него берётся
текущий рабочий каталог. Диалог выбора каталога не открывается.
...
`fs_tools.shared.log`): блок с меткой времени и строками тела:

- нормализатор — пары `old -> new` выполненных переименований либо `(изменений нет)`;
