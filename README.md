...
`.fs-log` в проверяемом каталоге (тот же формат, что у `fs-normalizer`), и только
...
│   ├── log.py                  # журнал .fs-log (зеркало fs-normalizer)
...
1. **дописывает журнал** `.fs-log` в проверяемом каталоге (метка времени + список
   путей). Формат совпадает с `fs-normalizer`: если обе утилиты запускаются над
   одним каталогом, они дополняют **один и тот же** файл как единая система;
