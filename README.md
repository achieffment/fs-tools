...
- `[defaults].apply_root` — опционально: путь каталога, который реально
  ...
  проверки = каталог конфига (текущее поведение). `.fs-log` при этом остаётся
...
путь, для контентных — ожидание/факт) дописывается в `.fs-log`, см. раздел
[«Журнал `.fs-log`»](#журнал-fs-log).
