...
- **Сохранение include-каталогов**: при `[[backup]]` опустевшие каталоги, покрытые
  include-правилами профиля (например `**/Back/`), не удаляются после offload — удаляется
  только подтверждённо переданное содержимое.
