...
- **нормализация имён** (`fs-nrm`) — рекурсивно приводит имена файлов и папок к единому
...
- **проверка структуры** (`fs-chk`) — рекурсивно сверяет дерево с правилами `.fs-check`
...
После установки доступны команды `fs-nrm`, `fs-chk`, `fs-tools` (и `python -m fs_tools`).
...
```bash
bin/normalize.sh [каталог]      # Linux/macOS (терминал) → fs-nrm
bin/check.sh [каталог]          # Linux/macOS (терминал) → fs-chk
bin/normalize.command           # macOS (двойной клик в Finder)
bin/check.command               # macOS (двойной клик в Finder)
bin/normalize.bat [каталог]     # Windows → fs-nrm
bin/check.bat [каталог]         # Windows → fs-chk
```
...
```bash
fs-nrm                          # нормализация: выбрать каталог в диалоге
fs-nrm /path/to/dir             # без диалога (для cron/планировщика)

fs-chk                          # проверка: выбрать каталог в диалоге
fs-chk /path/to/dir             # без диалога
