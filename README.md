...
- Фильтр путей `.fs-ignore` в стиле `.gitignore` (в корне проекта): обычная строка исключает объект из нормализации, строка с `!` — возвращает (override). Полный синтаксис gitignore (`*`, `**`, `?`, `[abc]`, завершающий `/`, якоря), порядок строк значим. Без правила-`!` внутрь исключённых каталогов не заходим, такие объекты не учитываются в подсчёте (см. «Фильтр путей»).
...
├── .fs-ignore                        # фильтр путей в стиле .gitignore (gitignore, создаётся пустым)
├── normalizer/
...
Чтобы исключить отдельные каталоги или файлы из нормализации, перечислите их в файле `.fs-ignore` в корне проекта (рядом с `normalize_fs.py`). Формат — **как у `.gitignore`** (под капотом библиотека [`pathspec`](https://pypi.org/project/pathspec/)): обычная строка исключает, строка с ведущим `!` — возвращает (override) ранее исключённое. Файл создаётся автоматически (пустым) при первом запуске обёртки и добавлен в `.gitignore`, поэтому у каждого пользователя свой локальный список.

Пути сопоставляются **относительно нормализуемого каталога** (как `.gitignore` в корне репозитория). Пустые строки и строки-комментарии (`#`) игнорируются. Пример: исключаем архивы и сборки, но возвращаем папки `Data` и `.keep`-файлы внутри них.
...
- **Отрицание (`!`) и порядок.** Строка с `!` возвращает к нормализации то, что убрала предыдущая строка. Решение по каждому пути принимает последняя совпавшая строка (как в git). Это позволяет «исключить каталог, но вернуть в нём отдельные файлы».
...
# Фильтр необязателен: load_fs_ignore вернёт None, если .fs-ignore нет.
ignorer = load_fs_ignore(Path("/path/to/project"))    # стиль .gitignore
