...
│   ├── filesystem.py    # обход и переименование (deepest-first)
...
│   ├── notify.py        # веб-хук + .env (ленивые requests/dotenv)
...
- **Ресурсы и `.env` — не от `__file__`**: `pick_folder.ps1` грузится через
  `importlib.resources.files("fs_tools.shared")`; путь к `.env` — через
...
Раскладка зеркалит пакет: `tests/shared/`, `tests/normalizer/` (+`rules/`),
`tests/checker/`. Фикстуры: `nn` — `tests/normalizer/conftest.py`; `make_tree`,
`write_rule` — `tests/checker/conftest.py`. Идемпотентность нормализатора проверяй на
копии `examples/normalizer/` (исходник под git не трогаем).
