"""Пакет односторонней синхронизации каталога с сервером (ПК → сервер) через rsync.

Читает .fs-sync.toml, транслирует правила include/exclude в фильтры rsync (сам rsync —
единственный источник истины для сопоставления путей), запускает rsync, разбирает
итог, пишет журнал .fs-log и шлёт веб-хук. Публичное API: импортируйте отсюда, а не
